...
int main(int argc, char** argv) {
  ...
  // Percentili: solo sulla coppia selezionata (per la scala colori del plot)
  std::vector<double> effs;
  for (auto const& [id, eff] : efficiency_sum) {
    if (!filtered_config.count(id)) continue;
  ...
  // ===============================
  auto pair_stats = compute_pair_ranking(efficiency_sum, n_entries, config_map, !is_opt_mode);
  {
